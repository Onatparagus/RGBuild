...
public class PointedSpectriumFeature extends Feature<PointedSpectriumConfiguration> {
    public PointedSpectriumFeature(Codec<PointedSpectriumConfiguration> p_191067_) {
        super(p_191067_);
    }

    public boolean place(FeaturePlaceContext<PointedSpectriumConfiguration> p_191078_) {
        LevelAccessor levelaccessor = p_191078_.level();
        BlockPos blockpos = p_191078_.origin();
        RandomSource randomsource = p_191078_.random();
        PointedSpectriumConfiguration pointedspectriumconfiguration = p_191078_.config();
        ...
        if (optional.isEmpty()) {
            ...
            createPatchOfDripstoneBlocks(levelaccessor, randomsource, blockpos1, pointedspectriumconfiguration);
            int i = randomsource.nextFloat() < pointedspectriumconfiguration.chanceOfTallerDripstone && DripstoneUtils.isEmptyOrWater(levelaccessor.getBlockState(blockpos.relative(optional.get()))) ? 2 : 1;
            SpectriteUtils.growPointedDripstone(levelaccessor, blockpos, optional.get(), i, false);
            return true;
        }
    }
    ...
    private static void createPatchOfDripstoneBlocks(LevelAccessor p_225194_, RandomSource p_225195_, BlockPos p_225196_, PointedSpectriumConfiguration p_225197_) {
        SpectriteUtils.placeDripstoneBlockIfPossible(p_225194_, p_225196_);

        for(Direction direction : Direction.Plane.HORIZONTAL) {
            if (!(p_225195_.nextFloat() > p_225197_.chanceOfDirectionalSpread)) {
                BlockPos blockpos = p_225196_.relative(direction);
                SpectriteUtils.placeDripstoneBlockIfPossible(p_225194_, blockpos);
                if (!(p_225195_.nextFloat() > p_225197_.chanceOfSpreadRadius2)) {
                    BlockPos blockpos1 = blockpos.relative(Direction.getRandom(p_225195_));
                    SpectriteUtils.placeDripstoneBlockIfPossible(p_225194_, blockpos1);
                    if (!(p_225195_.nextFloat() > p_225197_.chanceOfSpreadRadius3)) {
                        ...
                        SpectriteUtils.placeDripstoneBlockIfPossible(p_225194_, blockpos2);
                    }
                }
            }
        }
